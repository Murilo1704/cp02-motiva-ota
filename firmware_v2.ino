// PROJETO MOTIVA - CP02 | FIRMWARE 2.0 - Mediana, histerese e LED de estado
...
// Histerese sobre a mediana
const int LIMITE_ALERTA_CM = 16;   // >= 16 -> ALERTA
const int LIMITE_NORMAL_CM = 14;   // <= 14 -> NORMAL

enum EstadoVegetacao { NORMAL, ALERTA };

// Modo de teste (teclas 1, 2 e 3 no Serial)
const int TESTE_ALERTA[NUM_LEITURAS]        = {18, 12, 17, 19, 16};  // mediana 17
const int TESTE_INTERMEDIARIO[NUM_LEITURAS] = {15, 13, 16, 14, 15};  // mediana 15
const int TESTE_NORMAL[NUM_LEITURAS]        = {12, 14, 11, 13, 19};  // mediana 13
...
const int *conjuntoTestePendente = nullptr;
const int *conjuntoTesteSessao   = nullptr;
const char *nomeTestePendente    = "";
...
// Verde = NORMAL, vermelho = ALERTA
void atualizarLedEstado() {
  if (estadoAtual == ALERTA) definirCorLed(true, false, false);
  else                       definirCorLed(false, true, false);
}
...
void imprimirBanner() {
  ...
  Serial.println(F("Novidades da 2.0: ordenacao, mediana, histerese e LED de estado"));
  Serial.println(F("Histerese: mediana >= 16 cm -> ALERTA | <= 14 cm -> NORMAL | entre -> mantem"));
  Serial.print(F("Estado inicial: "));
  Serial.println(nomeEstado(estadoAtual));
  Serial.println(F("Modo de teste (digite no Serial): 1 = ALERTA | 2 = intermediario | 3 = NORMAL"));
}

void imprimirVetor(const char *rotulo, const int valores[], int n) {
  Serial.print(rotulo);
  for (int i = 0; i < n; i++) {
...
void lerComandosSerial() {
  while (Serial.available() > 0) {
    ...
    switch (c) {
      case '1':
        conjuntoTestePendente = TESTE_ALERTA;
        nomeTestePendente = "mediana 17 cm (>= 16, espera ALERTA)";
        break;
      case '2':
        conjuntoTestePendente = TESTE_INTERMEDIARIO;
        nomeTestePendente = "mediana 15 cm (entre 14 e 16, espera manter estado)";
        break;
      case '3':
        conjuntoTestePendente = TESTE_NORMAL;
        nomeTestePendente = "mediana 13 cm (<= 14, espera NORMAL)";
        break;
      default:
        continue;
    }
    Serial.print(F("[TESTE] Proxima sessao usara leituras fixas: "));
    Serial.println(nomeTestePendente);
  }
}

// ---------------- MEDICAO E PROCESSAMENTO ----------------
...
// Bubble sort em uma COPIA, preservando a ordem original
...
EstadoVegetacao aplicarHisterese(int mediana, EstadoVegetacao anterior) {
  if (mediana >= LIMITE_ALERTA_CM) return ALERTA;
  if (mediana <= LIMITE_NORMAL_CM) return NORMAL;
  return anterior;   // 14 < mediana < 16: mantem o estado
}
...
void iniciarSessao(unsigned long agora) {
  ...
  conjuntoTesteSessao   = conjuntoTestePendente;
  conjuntoTestePendente = nullptr;
  ...
  Serial.println();
  Serial.print(F("Estado atual: "));
  Serial.println(nomeEstado(estadoAtual));
  if (conjuntoTesteSessao != nullptr) {
    Serial.print(F("[TESTE] Sessao com leituras fixas: "));
    Serial.println(nomeTestePendente);
  }
}

void realizarLeitura() {
  int valor = (conjuntoTesteSessao != nullptr)
              ? conjuntoTesteSessao[leiturasFeitas]
...
void processarSessao() {
  ordenarCopia(leituras, ordenadas, NUM_LEITURAS);
  ...
  Serial.println(F(" cm"));
  Serial.print(F("Mediana da sessao: "));
  Serial.print(mediana);
  Serial.println(F(" cm"));

  EstadoVegetacao anterior = estadoAtual;
  estadoAtual = aplicarHisterese(mediana, anterior);

  Serial.print(F("Histerese: "));
  if (mediana >= LIMITE_ALERTA_CM) {
    Serial.print(F("mediana >= 16 cm -> entrar em ALERTA"));
  } else if (mediana <= LIMITE_NORMAL_CM) {
    Serial.print(F("mediana <= 14 cm -> entrar/retornar a NORMAL"));
  } else {
    Serial.print(F("14 cm < mediana < 16 cm -> manter estado anterior"));
  }
  Serial.println();

  Serial.print(F("Estado: "));
  Serial.print(nomeEstado(anterior));
  Serial.print(F(" -> "));
  Serial.print(nomeEstado(estadoAtual));
  Serial.println(anterior == estadoAtual ? F("  (sem mudanca)") : F("  (MUDANCA DE ESTADO)"));

  atualizarLedEstado();
  Serial.print(F("LED: "));
  Serial.println(estadoAtual == ALERTA ? F("VERMELHO") : F("VERDE"));
...
// ---------------- OTA (mantida para futuras versoes) ----------------
